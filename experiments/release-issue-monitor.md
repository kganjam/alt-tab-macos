...
- Thumbnail IOSurface tally / leak (REL-100): NO leak. **3-hour soak (150 samples, 01:56→04:57)**: `surfaces` constant at 158 (= live `windows=164`), zero growth; RSS 630→529 MB (range 525–630, trended down, no upward creep); same PID throughout, no crash, no WindowServer abort. Holding ~158 IOSurface thumbnails bounded by the lifecycle fixes is stable over hours. (Multi-day confidence still comes from the daily `surfaces`/`windows` watch.)
...
| REL-102 | Un-completable capture retry-storm hangs/kills WindowServer | Windows whose SCK/CGS capture never returns a completion get re-dispatched forever (`rescueStuckInFlightLocked` re-queued at `now+1`), so ~10 dead captures continuously load WindowServer/replayd until it beachballs (2026-07-15 unlock) or is watchdog-killed (2026-07-28 07:14, replayd hit its 513-thread limit, all donated by AltTab). Signature: `THUMBCACHE inFlight` pinned at ~max for many minutes while `cgs`/`sck` rate collapses; WS RSS climbs; `WindowServer_*.userspace_watchdog_timeout.spin` present | Watch `THUMBCACHE`: `inFlight` must NOT stay pinned; `expired`/`recentExpiries`/`timingOut`/`quarantined` must stay ~0 in steady state; `captureLatencyMs` low (tens of ms). `ws-mem-watch.log` replayd thread count must stay low. Any `OVERLOAD`/`THUMBQUAR` (warn) marks the breaker/quarantine engaging — a sustained stream means captures aren't completing. Verify by re-enabling `bgThumbnailRefreshEnabled` and soaking | Fixed (branch `deploy-combined`, 311e6cc4/81ed2bf1/69b723d7): per-window timeout→quarantine, WindowServer probe-recovery circuit breaker (closed/open/halfOpen, O(1) replayd growth), and indefinite display-off/screen-lock capture hold. Verified healthy on CGS soak; SCK path uses same machinery but not yet stress-tested |
| REL-103 | Stale/blank thumbnails (Edge after restart; window just left via AltTab) | After a Mac or AltTab restart, Edge tiles show blank content or an older page; the window the user just alt-tabbed away from shows how it looked when they arrived. Cause 1 (not AltTab): Edge/Chromium doesn't paint occluded windows or lazily-restored tabs, so WindowServer — and every capture — holds a frozen frame. Cause 2 (AltTab): an AltTab switch never re-captured the source window (only the target got `noteRecentlyActive`; the on-show capture is dropped when the panel closes first), and the target was captured before it repainted | `AltTab --dump-thumbnails=<dir>` and compare PNGs against titles; after an AltTab switch the source's `thumbnailAgeMs` (`--detailed-list`) must be ~seconds; `CAPTURE session source capture` once per session; `THUMBFROZEN` fires at most once per hidden window per visibility epoch; `THUMBCACHE cgsCaptures` rate must not rise materially vs. baseline, and no `OVERLOAD`/`THUMBQUAR` | Event-driven freshness (flags default on): session-source capture at session start, 1.5s activation settle, title/resize content-change capture rate-limited to 15s/window with frozen detection. Frozen frames of never-shown Edge windows remain until the user shows them — unfixable by capture, by design |
| REL-104 | macOS 26+ screen-recording indicator always lit | The purple Control Center pill stays on permanently while AltTab runs. Measured 2026-09-18 (macOS 27.0 / 26A428): no longer SCK-only — the private `CGSHWCaptureWindowList` path lights it too, ~8–12s of linger per capture, so the hot-tier 60s cadence keeps it lit continuously. A/B: `thumbnailCaptureEnabled=false` → pill dark within ~12s; re-enabled → lit again (`sckCaptures=0` throughout, so CGS alone does it) | Menu-bar probe: crop the pill region and count saturated purple pixels with captures on vs off, or watch the pill while toggling `captureWindowsInBackground`. With that setting off the pill must stay dark while idle AND during a fast alt-tab that never shows a panel; it may light only while the switcher is on screen | Not suppressible — it is a privacy indicator and no window-capture API avoids it. Reduced to panel-only: the refresher tick and the session-source capture now honour `captureWindowsInBackground`. Zero-pill option is `hideThumbnails` (icons only) |
...
### REL-102 — Un-completable capture retry-storm hangs/kills WindowServer
- Symptom: the whole GUI goes dog-slow / beachballs, or the graphics session force-logs-out, after minutes-to-hours of normal use. Recovers when AltTab is restarted or `bgThumbnailRefreshEnabled` is turned off.
- Root cause: some windows (offscreen / Parallels Coherence / dead-surface) never return a capture completion. The shipped `ThumbnailCache.rescueStuckInFlightLocked` cleared such an in-flight capture after `inFlightTimeoutSec` (15s) and re-queued it at `now+1` **unconditionally** — so ~10 un-capturable windows were re-dispatched every cycle forever. `bgThumbnailMaxConcurrent` bounds AltTab's own in-flight count but does NOT bound WindowServer/replayd work: under SCK each abandoned capture strands a `replayd` thread that never returns, so the leak grows with time until replayd hits its 513-thread dispatch limit and its turnstile chain wedges WindowServer's main thread.
- Evidence:
  - 2026-07-15 ~23:44 (unlock beachball): `WindowServer_2026-07-15-234729_mac-2.cpu_resource.diag` — WS 65% CPU for 139s in the window-list capture path (`WSCaptureCreateIOSurfaceMachPortForWindowList`), attributed on-behalf-of old AltTab pid + replayd. `ws-mem-watch.log` showed `inFlight` climb to 8–10 and pin while `cgsCaptures` rate collapsed from ~500 to ~30 per 5 min across the screen-locked span.
  - 2026-07-28 07:14:35 (watchdog kill): `WindowServer_2026-07-28-071443_mac-2.userspace_watchdog_timeout.spin` — replayd 513 threads (401 on `com.screenCaptureKit.screenshotSessionQueue`), **all** donated by AltTab; chained through the replayd connection-manager turnstile to WindowServer, whose main thread was pinned 100% in SkyLight. `ws-mem-watch.log`: `inFlight=10` pinned for 90+ min, `sckCaptures` climbing then frozen at 07:04.
- Fix (branch `deploy-combined`; all flag-gated, default on):
  1. Per-window quarantine (`ThumbnailCache`): a timed-out capture backs off exponentially (1,2,4s…) and after `bgThumbnailCaptureTimeoutQuarantineThreshold` (3) consecutive timeouts is suspended for `bgThumbnailCaptureQuarantineMs` (300s); any completion/activation resets it. Logs `THUMBQUAR`; `timingOut`/`quarantined` in the `THUMBCACHE` statsLine.
  2. WindowServer probe-recovery circuit breaker (`BackgroundThumbnailRefresher.captureBudgetThisTick` + `ActiveWindowCaptures` latency/expiry telemetry): closed→open on a burst of capture expiries (`bgThumbnailOverloadExpiryThreshold` 5 in 60s) or high EWMA latency; open pauses ALL dispatch for `bgThumbnailOverloadCooldownMs` (15s); halfOpen dispatches exactly one probe and only recovers if it completes, re-opens if it expires/hangs — so stranded-thread growth is O(1) per cooldown, not O(time). Logs `OVERLOAD`; `captureLatencyMs`/`expired`/`recentExpiries` in `THUMBCACHE`.
  3. Display-off / screen-lock capture hold (`SleepWakeEvents` + `App.holdThumbnailCapturesForDisplayOff`): `screensDidSleep/Wake` + `com.apple.screenIsLocked/Unlocked` hold captures for the whole off/locked span (indefinite, not the fixed transition pause), fixing the herd-on-unlock beachball.
- Guardrail: in steady state with refresh ON, `THUMBCACHE` must show `inFlight` NOT pinned, and `expired`/`recentExpiries`/`timingOut`/`quarantined` ~0, `captureLatencyMs` in the tens of ms. A sustained stream of `OVERLOAD`/`THUMBQUAR` (warn) or a rising replayd thread count in `ws-mem-watch.log` means captures aren't completing — investigate before shipping. `defaults write com.lwouis.alt-tab-macos bgThumbnailRefreshEnabled -bool false` is the instant no-rebuild mitigation.
- Validation: re-enable `bgThumbnailRefreshEnabled`, soak ≥10 min at high window count, and confirm the guardrail metrics stay clean and WindowServer RSS + replayd thread count stay flat. Because the 2026-07-28 kill was SCK-driven and the current default backend is CGS (`thumbnailUseScreenCaptureKit` off), a dedicated soak with SCK ON is still required before trusting the SCK path — the breaker/quarantine wrap it but it has not been stress-tested.
