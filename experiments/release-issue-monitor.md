...
| REL-100 | Thumbnail capture IOSurface tally crashes WindowServer | WindowServer `abort()` via `WSIOSurfaceDebugTallyAndAbort`; whole graphics session dies; crash-report voucher shows `originatorName: AltTab`; `THUMBCACHE` `liveSurfaces` grows over a long session | Long session with many windows + Parallels Coherence; `THUMBCACHE` `liveSurfaces` must stay ≈ `bgThumbnailHotTierSize` and NOT grow with window count; minimized skipped; 30s reconcile releases closed windows | Fixed: non-hot tiers copied to detached malloc bitmaps (IOSurface released immediately), 3-tier cadence 5s/60s/300s, minimized-skip, background reconcile/zombie-GC, token+watchdog `ActiveWindowCaptures`, computed `CGS_CONNECTION` (13e532ed/3db59bf8). Long-run `liveSurfaces` watch still recommended |
